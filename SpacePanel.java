...
public class SpacePanel extends JPanel implements Runnable, KeyListener {
	...
	private SoundEffect explosionSound, pickupSound, gameoverSound;
	...
	public SpacePanel () {
		...
		setFocusable(true);
		requestFocus();
		explosionSound = new SoundEffect("audio/explosion.wav");
		pickupSound = new SoundEffect("audio/pickup.wav");
		gameoverSound = new SoundEffect("audio/game-over.wav");
	}
	public void addNotify () {
		...
		addKeyListener(this); // Receiving input from the keyboard
	}
	...
	public void run () {
		...
		if (background1 == null) background1 = new Generals().loadImg("/img/stage/stage8.jpg");
		...
		while (running) {
			...
			gameUpdate(); // Logic game update
			gameRender(); // Draw the game frame to the buffer
			gameDraw();   // Display a buffer to the screen
		...
		//		Credits message
		g.drawString("@PecintaNaskun", (width - length) / 2 + 270, (height - length) / 2 + 340);
	...
	private void gameRender() {
		//		Drawing a background of our game
